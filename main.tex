...
Evolutionary algorithms have been traditionally compared by taking into account quality of solutions and time required to reach them.  Although time to solution is indirectly meassured by analysing the number of generations computed, this is not always possible, given that a number of parameters may differ between algorithms evaluated, including population sizes or variable chromosomes required.  So, wall clock time is sometimes the only possible alternative to compare algorithms.

In this work we propose another important feature that is typically forgoten in the area:  power consumption.  As we will experimentally show, differences arise when comparisons take into account this important factor in nowadays environmentally concious society, so that unexpected conclusions may be reached when computing time is not the main goal.


\end{abstract}

\noindent \textbf{PALABRAS CLAVE:} Algoritmos Evolutivos, Consumo de potencia
