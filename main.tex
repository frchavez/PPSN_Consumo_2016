...
Evolutionary algorithms have been traditionally compared by taking
into account quality of solutions and time required to reach them.
Although time to solution is indirectly meassured by analysing the
number of generations computed, this is not always possible, given
that a number of parameters may differ between algorithms evaluated,
including population sizes or variable chromosomes required.  So,
wallclock time is sometimes the only possible alternative to compare 
algorithms.  % where and when? Are you framing the problem? - JJ

In this work we propose another important feature that is typically
forgoten in the area:  power consumption.  As we will show
experimentally, differences arise when comparisons take into account
this 
important factor in nowadays environmentally conscious society, so that
unexpected conclusions may be reached when computing time is not the
main goal.
% problem is, we can already expect that different devices will have
% different energy profiles. What is new about it?  - JJ
...
\sloppypar We acknowledge support from Spanish Ministry of Economy and Competitiveness and European Regional Development Fund (FEDER) under project EphemeCH (TIN2014-56494-C4-\{1,2,3\}-P), from Junta de Andaluc\'{\i}a under project DNEMESIS (P10-TIC-6083) and from Universidad de M\'alaga, Campus de Excelencia Internacional Andaluc\'{\i}a Tech.
