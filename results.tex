...
As described in the previous section, power consumption and computer time for each of the devices and algorithm tested, using different population size for each of the experiments, are reported in Table~\ref{T:r_obtained} and Figure~\ref{fig:powerenergy}.  The first thing that we may notice, is the differences in computing time among devices analysed, which corresponds with expectations:  small devices requires quite longer time to reach solutions, and this is typically the reason why raspberry pi and tablets are not frequently use as the hardware platform to run EAs, although they are useful when non-standard distributed models are analysed (such as pool-based models, \cite{pool}).

Nevertheless, we are considering a different point of view, and won't only focus on computing time.  Different features can thus be analyzed:  (i) device behavior when considering energy consumed by the algorithm per unit of time; (ii) total energy required to find a solution and (iii) the way population size influences the energy needs when looking for a solution.

If we focus first on the energy required by lilgp to be run in every device (see \ref{fig:powerenergy}), we notice that raspberry pi and handheld devices are the least demanding ones, requiring 20 times less energy to run the same algorithm when compared with more standard computers:  Mac, Laptop and blade system.
...
Finally, we can also analyse the influence of population sizes in both the time to reach solutions and the energy required for that.  In the experimental stage, the runs with smaller population sizes typically reach 500 generations -maximum allowed, given that they cannot find the solution with that small number of individuals.  Therefore, we will focus on the runs with larger population sizes and  the energy consumed to reach that solutions.  We see in figure \ref{fig:powerenergy} that it's better to use the 500 individuals instead of 1000 when considering energy, similarly as already described by Koza when considering time:  500 is an optimal size for the problem, and adding more individuals is a burden that implies larger computing time.  Nevertheless, what is of interest in this preliminary analysis, is the behavior of every device when this parameter is changed in this sequential version of the algorithm:  we have recorded a change in power consumption for the laptop, mac and blade system, while no change is found for raspberry and quite small changes for the tablet;  and this again benefits the latests (see figure \ref{fig:powerenergy}-a).  


MISSING CARLOS COMMENTS ON THE POWERLAW FIGURE
...
\begin{figure}[!t]
\subfloat[]{\includegraphics[width=.48\textwidth]{excess_power_log_2.eps}}~~
\subfloat[]{\includegraphics[width=.48\textwidth]{excess_energy_log_2.eps}}
...
\begin{figure}[!t]
\subfloat[]{\hspace{-1.1cm}\includegraphics[width=.585\textwidth]{fitness.eps}}~~
\subfloat[]{\includegraphics[width=.48\textwidth]{fitnessenergy.eps}}
